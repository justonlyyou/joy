/* 登陆失败原因代码 */
CREATE TABLE `code_login_state` (
  `CODE` varchar(2) NOT NULL COMMENT '代码',
  `TRANS` varchar(32) NOT NULL COMMENT '译文',
  PRIMARY KEY (`CODE`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='登陆状态代码';

insert into `code_login_state`(`CODE`,`TRANS`) values ('00','登陆成功');
insert into `code_login_state`(`CODE`,`TRANS`) values ('11','密码错误');
