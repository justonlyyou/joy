...
<%@ attribute name="origFmt" type="java.lang.String" required="false" description="时间原始格式，默认为yyyyMMddHHmmss"%>
...
<td>
	<input class="Wdate form-control joy-date-picker" value='${joyFn:formatDate(timeStart, origFmt, displayFmt)}' placeholder="${startPlaceholder}" title="${startPlaceholder}"
		onFocus="WdatePicker({dateFmt:'${displayFmt}',realFullFmt:'${fn:replace(origFmt, 'SSS', '000')}',firstDayOfWeek:${firstDayOfWeek},autoUpdateOnChanged:${autoUpdateOnChanged},vel:'_joy_id__${timeStartProperty}'})" />
</td>
<td>
&nbsp;&nbsp;-&nbsp;&nbsp;
</td>
<td>
	<input class="Wdate form-control joy-date-picker" value='${joyFn:formatDate(timeEnd, origFmt, displayFmt)}' placeholder="${endPlaceholder}"  title="${endPlaceholder}"
		onFocus="WdatePicker({dateFmt:'${displayFmt}',realFullFmt:'${fn:replace(origFmt, 'SSS', '000')}',firstDayOfWeek:${firstDayOfWeek},autoUpdateOnChanged:${autoUpdateOnChanged},vel:'_joy_id__${timeEndProperty}'})" />
</td>
