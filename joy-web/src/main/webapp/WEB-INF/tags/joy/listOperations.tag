...
<div class="joy-list-row-operations ${cssClass}">
	<c:if test="${showDetailOp == null || showDetailOp}">
		<i class="fa fa-list-alt" title="详情" onclick="${pageObj}.${detailFunc}('${id}')" ></i>&nbsp;
	</c:if>
	<c:if test="${showEditOp == null || showEditOp}">
		<i class="fa fa-edit" title="编辑" onclick="${pageObj}.${editFunc}('${id}')"></i>&nbsp;
	</c:if>
	
	<c:if test="${showDeleteOp == null || showDeleteOp}">
		<i class="fa fa-times" title="删除" onclick="${pageObj}.${deleteFunc}('${id}')"></i>&nbsp;
	</c:if>
	<c:if test="${exOp1CssClass != null}">
		<i class="${exOp1CssClass}" title="${exOp1Title}" onclick="${pageObj}.${exOp1Func}('${id}')"></i>&nbsp;
	</c:if>
	<c:if test="${exOp2CssClass != null}">
		<i class="${exOp2CssClass}" title="${exOp2Title}" onclick="${pageObj}.${exOp2Func}('${id}')"></i>&nbsp;
	</c:if>
</div>