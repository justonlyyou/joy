...
/**
 * A <code><em>Soft</em>HashMap</code> is a memory-constrained map that stores
 * its <em>values</em> in {@link SoftReference SoftReference}s. (Contrast this
 * with the JDK's {@link SoftHashMap SoftHashMap}, which uses weak references
 * for its <em>keys</em>, which is of little value if you want the cache to
 ...
 * Kabutz's and Sydney Redelinghuys's <a
 * href="http://www.javaspecialists.eu/archive/Issue015.html">publicly posted
 ...
 */
 class SoftHashMap<K, V> implements Map<K, V> {
	 ...
	 * of last access.
	 */
	private final Queue<V> strongReferences; // guarded by
	 ...
	 * <code>retentionSize</code> value. This number is intended to be a
	 ...
	 * with a default retention size of {@link #DEFAULT_RETENTION_SIZE
...
//		System.out.println(map.get("1"));
			...
			for (int i = 0; i < 1000; i++) {
				...
				d.put(new byte[500][500], new byte[500][500]);
				for (int j = 0; j < 100; j++) {
					maps.add(d);
				}
