...
import com.kvc.joy.swing.LazyActionMap;
 ...
 */
public class TopTableHeaderUI extends TableHeaderUI {
...
//
    ...
    protected TopTableHeader header;
    ...
    protected Rectangle draggedCellRect;
    ...
    private class FocusHandler implements FocusListener {
        ...
        private void repaintHeader(Object source) {
            if (source instanceof JTableHeader) {
                ...
                TopTableHeaderUI ui = (TopTableHeaderUI) th.getUI();
                if (ui == null) {
                    return;
                }
    ...
    private class MouseInputHandler implements MouseInputListener {
        ...
        public void mouseClicked(MouseEvent e) {
            if (header.getCursor() != resizeCursor &&
                    e.getClickCount() % 2 == 1 && SwingUtilities.isLeftMouseButton(e)) {
                //如果点击在拖动的界限发生则不在生成排序
                if (header.getCursor() != resizeCursor) {
...
//
    public static ComponentUI createUI(JComponent h) {
        return new TopTableHeaderUI();
    }

//  Installation
    public void installUI(JComponent c) {
        header = (TopTableHeader) c;
        header.setLayout(new TopTableHeaderUILayout());
        installComponents();
        installDefaults();
        installListeners();
...
//        header.add(rendererPane);
    ...
    protected void installKeyboardActions() {
        ...
        LazyActionMap.installLazyActionMap(header, TopTableHeaderUI.class,
...
//
    ...
    void selectColumn(int newColIndex, boolean doScroll) {
        Rectangle repaintRect = getHeaderContentRect(selectedColumnIndex);
        header.repaint(repaintRect);
        selectedColumnIndex = newColIndex;
        repaintRect = getHeaderContentRect(newColIndex);
        header.repaint(repaintRect);
        if (doScroll) {
...
//
    public void paint(Graphics g, JComponent c) {
        ...
        if (draggedColumn != null) {
            ...
            //计算绘制填充矩形的范围
            draggedCellRect = getHeaderContentRect(draggedColumnIndex);
    ...
    protected Component getColumnRenderer(int columnIndex) {
    ...
    protected void paintCell(Graphics g, Rectangle cellRect, int columnIndex) {
        Component component = getColumnRenderer(columnIndex);
    ...
    //CKCS mark  获得某列表头实际的标题绘制部分的高度
    //并没有包含过滤等子主键的高度
    protected Rectangle getHeaderContentRect(int calIndex) {
        Rectangle repaintRect = header.getHeaderRect(calIndex);
        repaintRect.height = getHeaderContentHeight();
        return repaintRect;
    }
    ...
    private Dimension createHeaderSize(long width) {
        ...
        Insets inset = header.getInsets();
        //获得最大的过滤组件的高度
        int heigth = 0;
        if (header.isShowFilters()) {
            if (header.getColumnModel() instanceof TopTableColumnModel) {
                heigth = ((TopTableColumnModel) header.getColumnModel()).getMaxFilterHeight();
            }
        }
//        System.out.println("createHeaderSize 从columnMode 获得的最大的过滤器的高度 = " + heigth);
        //加上内容(字体)的高度
        heigth += getHeaderContentHeight();
//        System.out.println("createHeaderSize 最后获得的最大的表头的高度 = " + heigth);
        return new Dimension((int) width + inset.left + inset.right, heigth + inset.top + inset.bottom);
    }
    ...
    private static class Actions extends AbstractAction implements UIResource {
        ...
        public boolean isEnabled(Object sender) {
            if (sender instanceof JTableHeader) {
                ...
                String key = getValue(NAME).toString();
                TopTableHeaderUI ui = (TopTableHeaderUI) th.getUI();
        ...
        private boolean maybeMoveColumn(boolean leftArrow, JTableHeader th,
                TopTableHeaderUI ui, boolean doIt) {
    ...
    //CKCS mark  表头布局管理
    private class TopTableHeaderUILayout implements LayoutManager {

        public TopTableHeaderUILayout() {
        }
        ...
        public void layoutContainer(Container parent) {
            ...
            if (header.getColumnModel() instanceof TopTableColumnModel) {
                TopTableColumnModel columnModel = (TopTableColumnModel) header.getColumnModel();
                //先排列过滤组件
                //过滤器的排列顺序，与列的排列顺序必须一致
                List<FilterComponent> filterList = columnModel.getFilters();
                if (filterList.size() > 0) {
                    FilterComponent filter = null;
                    Rectangle rec = null;
                    int maxHeight = columnModel.getMaxFilterHeight();
                    for (int index = 0; index < filterList.size(); index++) {
                        filter = filterList.get(index);
                        if (filter != null) {
                            //是否存在做个件
                            //拖动列的过滤组件
                            if (columnModel.getColumn(index) == header.getDraggedColumn()) {
                                rec = draggedCellRect;
                                //将拖动列提到最后绘制，以便覆盖为被拖动的组件
                                header.setComponentZOrder(filter, 0);
                            } else {
                                rec = getHeaderContentRect(index);
                            }
                            filter.setBounds(inset.left + rec.x, inset.top + rec.height, rec.width - inset.right, maxHeight - inset.bottom);
                        }
                    }
                }
            }
