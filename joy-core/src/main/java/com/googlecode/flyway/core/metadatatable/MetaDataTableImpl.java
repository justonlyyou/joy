...
public class MetaDataTableImpl implements MetaDataTable {
	...
	private String versionDomain; //diff add
     ...
     */
//    private boolean upgraded; //diff comment
     ...
     */
//    private final MigrationResolver migrationResolver; //diff comment
    ...
    public MetaDataTableImpl(DbSupport dbSupport, Table table, MigrationResolver migrationResolver) {
        ...
        this.table = table;
//        this.migrationResolver = migrationResolver; //diff comment
    }
    
    public MetaDataTableImpl(DbSupport dbSupport, Table table, MigrationResolver migrationResolver, String sqlMigrationPrefix) {
		this(dbSupport, table, migrationResolver);
		this.versionDomain = sqlMigrationPrefix; //diff add
    }
    ...
    private void createIfNotExists() {
    	// diff comment all code in this method
...
//        LOG.debug("Metadata table " + table + " created.");
    ...
    public void addAppliedMigration(AppliedMigration appliedMigration) {
        ...
        try {
            ...
            jdbcTemplate.update("UPDATE " + table
                    ...
                    + " + 1 WHERE " + dbSupport.quote("version_rank") + " >= ?" + getVesionDomainCondition(), versionRank); // diff modify
            jdbcTemplate.update("INSERT INTO " + table
                    + " (" + dbSupport.quote("version_rank")
                    + "," + dbSupport.quote("installed_rank")
                    + "," + dbSupport.quote("version_domain") //diff add
    ...
    private int calculateInstalledRank() throws SQLException {
    	int currentMax = jdbcTemplate.queryForInt("SELECT MAX(" + dbSupport.quote("installed_rank") + ")"
                + " FROM " + table + getVesionDomainWhereClause()); //diff modify
        return currentMax + 1;
    }
    ...
    private int calculateVersionRank(MigrationVersion version) throws SQLException {
    	List<String> versions = jdbcTemplate.queryForStringList("select " + 
        		dbSupport.quote("version") + " from " + table + getVesionDomainWhereClause()); //diff modify
    ...
    private List<AppliedMigration> findAppliedMigrations(MigrationType... migrationTypes) {
        ...
        if (migrationTypes.length > 0) {
            ...
            query += ")";
            query += getVesionDomainCondition();  // diff add
        } else {
        	query += getVesionDomainWhereClause();  // diff add
        }
    ...
    public void repair() {
        ...
        try {
        	int failedCount = jdbcTemplate.queryForInt("SELECT COUNT(*) FROM " + table
                    + " WHERE " + dbSupport.quote("success") + "=" + dbSupport.getBooleanFalse() + getVesionDomainCondition()); //diff modify
            if (failedCount == 0) {
                LOG.info("Repair of metadata table " + table + " not necessary. No failed migration detected.");
                return;
            }
    ...
    public boolean hasSchemasMarker() {
        ...
        try {
            int count = jdbcTemplate.queryForInt(
            		"SELECT COUNT(*) FROM " + table + " WHERE " + dbSupport.quote("type") + "='SCHEMA'" + getVesionDomainCondition()); //diff modify
            return count > 0;
        } catch (SQLException e) {
    ...
    public boolean hasInitMarker() {
        ...
        try {
            int count = jdbcTemplate.queryForInt(
            		"SELECT COUNT(*) FROM " + table + " WHERE " + dbSupport.quote("type") + "='INIT'" + getVesionDomainCondition()); //diff modify
            return count > 0;
        } catch (SQLException e) {
    ...
    public boolean hasAppliedMigrations() {
        ...
        try {
            int count = jdbcTemplate.queryForInt(
            		"SELECT COUNT(*) FROM " + table + " WHERE " + dbSupport.quote("type") + " NOT IN ('SCHEMA', 'INIT')" + getVesionDomainCondition()); //diff modify
            return count > 0;
        } catch (SQLException e) {
 ...
 // diff add
    private String getVesionDomainWhereClause() {
    	return " WHERE 1=1 " + getVesionDomainCondition();
    }
    
    // diff add
    private String getVesionDomainCondition() {
    	return " AND " + dbSupport.quote("version_domain") + " ='" + versionDomain + "'";
    }
}
